...
def init_db():
    ...
    # Tabla de partidos (Ahora incluye Fecha)
    cursor.execute("""
    ...
    # Tabla de jugadores (Ahora incluye a qué Equipo pertenece)
    cursor.execute("""
...
tabs = st.tabs(["1. Registrar Partido", "2. Cargar Alineación", "3. Base de Datos"])
...
with tabs[0]:
    ...
    with st.form("form_partido"):
        ...
        with col1:
            ...
            url_drive = st.text_input("Enlace del Video en Drive (Opcional por ahora)")
        with col2:
        ...
        if guardar_partido:
            ...
            conn.commit()
            conn.close()
            st.success(f"Partido entre {equipo_local} y {equipo_visitante} guardado con éxito.")
...
with tabs[1]:
    ...
    conn = get_connection()
    df_partidos = pd.read_sql("SELECT id, fecha, equipo_local, equipo_visitante, formato FROM partidos", conn)
    
    if df_partidos.empty:
        st.warning("Primero debes registrar un partido en la pestaña 1.")
    else:
        # Crear un selector legible para los partidos
        opciones_partido = {
            row['id']: f"{row['fecha']} | {row['equipo_local']} vs {row['equipo_visitante']} ({row['formato']})" 
            for _, row in df_partidos.iterrows()
        }
        partido_sel_id = st.selectbox("1. Selecciona el partido:", options=list(opciones_partido.keys()), format_func=lambda x: opciones_partido[x])
        
        # Obtener los nombres de los equipos del partido seleccionado para el selector
        ...
        with st.form("form_jugador"):
            equipo_seleccionado = st.radio("¿A qué equipo pertenece el jugador?", equipos_disponibles, horizontal=True)
            
            col_a, col_b, col_c = st.columns(3)
            with col_a:
                es_no_name = st.checkbox("Solo tengo el número (No Name)")
                nombre_input = st.text_input("Nombre del jugador", disabled=es_no_name)
                nombre_final = "No Name" if es_no_name or not nombre_input.strip() else nombre_input
                
            with col_b:
                dorsal = st.number_input("Número (Dorsal)", min_value=1, max_value=99, value=10)
                posicion = st.selectbox("Posición", ["Portero", "Defensa", "Medio", "Delantero"])
                
            with col_c:
                minuto_entrada = st.number_input("Minuto entra", min_value=0, max_value=120, value=0)
                es_cambiado = st.checkbox("¿Salió de cambio?")
                minuto_salida = st.number_input("Minuto sale", min_value=1, max_value=120, value=90) if es_cambiado else None
            ...
            if guardar_jugador:
                ...
                cursor.execute(
                    ...
                    (partido_sel_id, equipo_seleccionado, nombre_final, dorsal, posicion, minuto_entrada, minuto_salida)
                )
                conn.commit()
                st.success(f"Jugador #{dorsal} ({nombre_final}) agregado al equipo: {equipo_seleccionado}.")
